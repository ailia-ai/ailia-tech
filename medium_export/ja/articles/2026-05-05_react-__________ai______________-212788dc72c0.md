...
**対策:** LangGraph 等のフレームワークに合わせ、ツール引数を Gemma4のツール呼び出し形式に統一しました。
...
**対策:** Gemma4のThinkingモードを有効にし、Thoughtについてはプロンプトではなく、Thinkingに任せることにしました。これは、The Format Taxにおいて、いきなりjsonを出力すると精度が低下するが、Thinkingで考えをテキストを生成した後にjsonを出力すると精度が改善するという研究に基づいています。
