...
ailia DX Insightは、2026年夏にReACTエージェント機能の搭載を予定しており、ChatGPT5とGemma4 E4Bの両方で統一的にReACTを実装しました。ローカルLLMでも性能を出すため、Gemma4のプロンプトフォーマットに準拠し、適切にtool useできる形でReACTを実装しています。
...
### (1) ツール引数の入力形式: プレーンテキスト vs Native Format
...
# OK: プレーンテキスト - 本来はURLだけが来てほしい  
Action Input: https://example.com
```

**対策:** ツール引数を Gemma4のツール呼び出し形式に統一することで、LLMの出力がフォーマットを満たしやすくしました。
...
final parseErrorMessage =  
    ...
    'Or provide your final answer as plain text without any tool_call tags.';  
conversationBuffer  
  ..writeln()  
  ..writeln(parseErrorMessage);
```
...
Gemma4 等のローカル LLM に Action: を要求すると、Gemma4のネイティブフォーマットである call:tool\_name{…} でツールを呼び出してしまう問題がありました。

**対策:** Gemma4のプロンプトの書式に合わせてツールを定義し、Gemma4のネイティブフォーマットでのツール呼び出しを受け入れるようにしました。

[## Gemma 4 のプロンプトの書式設定 | Google AI for Developers

### Gemma 4 以降では、新しい制御トークンが導入されています。Gemma 3 以前については、 以前のドキュメント をご覧ください。 以降のセクションでは、Gemma 4…

ai.google.dev](https://ai.google.dev/gemma/docs/core/prompt-formatting-gemma4?hl=ja&source=post_page-----212788dc72c0---------------------------------------)

### (10) フォーマット遵守能力の限界

小規模 LLM は ReACT フォーマット (Thought: → Action: → Action Input:) に正確に従えないことがあります。

**対策:** Gemma4のThinkingモードを有効にし、Thoughtについてはプロンプトではなく、Thinkingに任せることにしました。これは、The Format Taxにおいて、直接jsonを出力すると精度が低下するが、Thinkingで考えのテキストを生成した後にjsonを出力すると精度が改善するという研究に基づいています。

[## The Format Tax

### Asking a large language model to respond in JSON should be a formatting choice, not a capability tax. Yet we find that…

arxiv.org](https://arxiv.org/abs/2604.03616?source=post_page-----212788dc72c0---------------------------------------)
